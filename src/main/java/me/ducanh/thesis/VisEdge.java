...
//  public Path path;
    ...
    public void init(String label, VisVertex source, VisVertex target) {
        ...
        draw();
        source.getCenterXProperty().addListener((oldV, newV, change) -> draw());
        source.getCenterYProperty().addListener((oldV, newV, change) -> draw());
        target.getCenterXProperty().addListener((oldV, newV, change) -> draw());
        target.getCenterYProperty().addListener((oldV, newV, change) -> draw());
    ...
    private void draw() {
        ...
        if (this.source == this.target) {
            ...
            cubicCurve.setStartX(source.getCenterX()-source.getRadius()*Math.cos(Math.toRadians(170.0D)));
            cubicCurve.setStartY(source.getCenterY()-source.getRadius()*Math.sin(Math.toRadians(170.0D)));
            cubicCurve.setEndX(target.getCenterX()-target.getRadius()*Math.cos(Math.toRadians(10.0D)));
            cubicCurve.setEndY(target.getCenterY()-target.getRadius()*Math.sin(Math.toRadians(10.0D)));
...
//    this.attachedLabel = label;
    ...
    public void bindArrow(SmartArrow arrow) {
        ...
        arrow.translateXProperty().bind(target.getCenterXProperty());
        arrow.translateYProperty().bind(target.getCenterYProperty());
